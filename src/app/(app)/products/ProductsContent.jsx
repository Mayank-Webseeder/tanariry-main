...
//   // NEW: Callback for subcategory card click (instant update on Products page)
...
//       {/* Occasion Cards Section - Now with instant filtering */}
...
export default function ProductsContent() {
  ...
  const [products, setProducts] = useState([]);
  const [categories, setCategories] = useState([]);
  const [subCategorySlugMap, setSubCategorySlugMap] = useState({});
  const [subCategoryIdToSlug, setSubCategoryIdToSlug] = useState({});
  const [totalProducts, setTotalProducts] = useState(0);
  const [loading, setLoading] = useState(true);
  const [loadingCats, setLoadingCats] = useState(true);
  const [error, setError] = useState(null);
        ...
        cats.forEach(cat => {
          cat.subCategories?.forEach(sub => {
            const slug = sub.name
              ...
              .replace(/[^a-z0-9]+/g, '-')
              .replace(/^-+|-+$/g, '');
            slugToId[slug] = sub._id;
            idToSlugMap[sub._id] = slug;
          });
        });

        setSubCategorySlugMap(slugToId);
        setSubCategoryIdToSlug(idToSlugMap);
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      // Subcategory filter (slug → id)
      if (selectedSubCategory !== "all") {
        const subCatId =
          subCategorySlugMap[selectedSubCategory] || selectedSubCategory;
        filtered = filtered.filter(p => p.subCategoryId === subCatId);
      }
...
useEffect(() => {
  if (loadingCats) return; // wait for slug map
  ...
  if (selectedCategory !== "all") {
    const cat = categories.find(c => c._id === selectedCategory);
    if (cat) params.set("category", toSlug(cat.name));
  }

  if (selectedSubCategory !== "all") {
    const slug = subCategoryIdToSlug[selectedSubCategory] || selectedSubCategory;
  ...
  router.replace(queryStr ? `?${queryStr}` : "/products", { scroll: false });
}, [selectedCategory, selectedSubCategory, sortBy, categories, subCategoryIdToSlug, loadingCats]);
